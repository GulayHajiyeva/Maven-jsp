...
public class StudentRepo implements CommonRepo<Student> {
    ...
    public void update(Student obj) {
        try (Connection connection = MyDatabase.connect()) {
            ...
            PreparedStatement statement = connection.prepareStatement(
                    "update student set name=?,surname=?,email=?,age=?,univeristy=? where id=?"
            );
    ...
    public void insert(Student obj) {
        try (Connection connection = MyDatabase.connect()) {
            PreparedStatement statement = connection.prepareStatement("insert into student(name,surname,email,age<university) values(?,?,?,?,?)");
            statement.setString(1, obj.getName());
            statement.setString(2, obj.getSurname());
            statement.setString(3, obj.getEmail());
            statement.setInt(4, obj.getAge());
    ...
    public List<Student> getList(String name,String surname) {
        List<Student> result = new ArrayList<>();
        try (Connection connection = MyDatabase.connect()) {
            PreparedStatement statement = connection.prepareStatement("select * from student where name=? or surname=?");
